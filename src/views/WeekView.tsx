...
import { getTimeFormat, getTimeFormatFull } from '../lib/date';
...
interface WeekViewProps {
  ...
  readonly?: boolean;
  language?: string;
  /** Hour (0-23) the calendar initially scrolls to on mount. Defaults to 8. */
  initialScrollHour?: number;
  /**
   * When true, time flows bottom-to-top (23:00 at top → 00:00 at bottom).
   * When false (default), time flows top-to-bottom (00:00 at top → 23:00 at bottom).
   */
  reverseTime?: boolean;
}
...
export const WeekView: React.FC<WeekViewProps> = ({
  ...
  // Hours array: reversed shows 23→0, normal shows 0→23
  const hours = reverseTime
  ...
  // Scroll to the configured start hour on mount
  React.useEffect(() => {
    if (scrollContainerRef.current) {
      // Reversed mode: initialScrollHour is near the bottom of the grid, so offset accordingly
      scrollContainerRef.current.scrollTop = reverseTime
        ? (23 - initialScrollHour) * hourHeight
        : initialScrollHour * hourHeight;
    }
  }, []);
  ...
  // Time formats driven by the active language (24h vs 12h AM/PM)
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-background border-[0.5px] border-border/50 rounded-2xl overflow-hidden min-w-[800px] md:min-w-0 shadow-sm">
      {/* Scrollable Container — flex-1 + min-h-0 prevents unbounded growth */}
      <div
      ...
      >
            ...
            {weekDays.map((day, dayIndex) => {
                ...
                <div key={day.toISOString()} className={cn("relative h-full", dayIndex > 0 && "border-l-[0.5px] border-border/30")}>
                    ...
                    {dayEvents.map((event) => {
                        ...
                        // In reversed mode, the event's visual top corresponds to the END time's position
                        const top = reverseTime
                          ? (1440 - startMinutes - durationMinutes) / 60 * hourHeight
                    ...
                    <div
                        ...
                        style={{
                          top: reverseTime
                            ? `${(1440 - (zonedNow.getHours() * 60 + zonedNow.getMinutes())) / 60 * hourHeight}px`
                            : `${(zonedNow.getHours() * 60 + zonedNow.getMinutes()) / 60 * hourHeight}px`
                        }}
                    >
        ...
        <div 
            ...
            style={{
                top: reverseTime
                  ? `${(1440 - (zonedNow.getHours() * 60 + zonedNow.getMinutes())) / 60 * hourHeight + 80}px`
                  : `${(zonedNow.getHours() * 60 + zonedNow.getMinutes()) / 60 * hourHeight + 80}px`,
            }}
        >
